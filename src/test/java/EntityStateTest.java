...
public class EntityStateTest {
    ...
    @BeforeEach
        //각각 메서드 실행전에 실행되는 메서드
    void setUp() {
        //Persistence클래스에 의해 엔티티매니저팩토리 생성
        emf = Persistence.createEntityManagerFactory("memo");
        em = emf.createEntityManager(); //엔티티 매니저 만들기
    }
    ...
    void test1() {
        ...
        try {
            ...
            Memo memo = new Memo(); // 비영속 상태(Trensient) : 영속성 컨텍스트(JPA)가 관리하지 않는 상태
            memo.setId(1L);
    ...
    void test5() {
        ...
        try {
            ...
            Memo mergedMemo = em.merge(memo); //memo가 없으므로, 새롭게 받아서 mergedMemo 에 반환

            //memo를 저장하는 것이 아닌 새롭게 받아온 mergedMemo가 영속성 컨텍스트에 올라감
            System.out.println("em.contains(memo) = " + em.contains(memo)); //memork 영속성 컨테이너안에 있는가? false
            System.out.println("em.contains(mergedMemo) = " + em.contains(mergedMemo)); //mergedMemo가 영속성 컨테이너안에 있는가?? true
